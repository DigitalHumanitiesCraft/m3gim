...
- Selbstbezügliche Korrespondenzbeziehungen (Befund der Vokabular-Formalisierung, siehe [vocabulary-derivation-findings.md](vocabulary-derivation-findings.md) § 4). Die Pipeline setzte die Nachlassbildnerin fest als Subjekt jeder AgRelOn-Beziehung. War sie zugleich Adressatin oder Absenderin des Dokuments, entstand eine Korrespondenzbeziehung von ihr zu sich selbst, mit identischer Wikidata-Kennung auf beiden Seiten (Beispiel `UAKUG/NIM_011 7`). In einem persönlichen Nachlass ist das der Regelfall. Entschieden und umgesetzt mit E-129, solche Beziehungen werden unterdrückt, während die Rollenzuordnung als `m3gim:hasAssociatedAgent` samt Quellzeile am Record bleibt. Offen bleibt, ob die tatsächliche Gegenseite als Beziehungsziel nachgezogen wird, geführt in [architecture-decisions.md](architecture-decisions.md) § Offene Entscheidungen unter AgRelOn-Granularität.
