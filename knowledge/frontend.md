...
- **Leere Perioden sichtbar** (Session 36, M2): `PERIOD_ORDER` ist Modul-Konstante, die Render-Schleife iteriert alle 8 Perioden; nicht belegte erscheinen als gedimmte Zeile („Kein bearbeitetes Material erfasst") und sind nicht klickbar. `Undatiert` wird weiterhin nur bei Belegung gerendert. Dichte-Mikro-Balken (CSS, `.chronik-period__density`) skaliert relativ zum max. Periodenwert. Karriere-Notizen tragen Dotted-Underline + Tooltip als Editorial-Markierung. Log-Stempel erweitert um `perioden-leer:N`.
