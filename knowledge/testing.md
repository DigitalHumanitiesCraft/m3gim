...
8. **Mobilitäts-Karten-Canary** (E-113): nach dem Klick auf den Mobilitäts-Tab wartet der Check auf den asynchronen Geometrie-Load (`loadCountries().then(...)`) und prüft, dass die D3-geo-Karte real zeichnet — Stadt-Knoten (`.mob-nodes g.mob-node` ≥ 5), Pfad-Pfeile (`.mob-arcs path` ≥ 1), Sicht-Filter (`.mob-legend__item` ≥ 5) und Ländergeometrie (`.mob-land path` ≥ 50), ohne neue Konsolenfehler. Harter FAIL, nicht WARN: eine still leer rendernde Karte (fehlende Geometrie, d3-Ausfall, Projektions-Bug) ist genau die Regression, die der logStamp-Check verfehlt, weil der Stempel synchron vor dem Async-Draw geschrieben wird.
