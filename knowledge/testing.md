...
Chronik data regressions verify source-specific document/statement separation in `NIM_023_5`, the Zürich context of `NIM_007_11`, undated contractual places in `NIM_005_16`/`NIM_005_17`, identity preservation, typed parts and exact evidence. Pure calendar checks cover precision, leap years, ranges, invalid/reversed values, qualifiers, natural grouping, coarse-date preservation, compressed gaps and inverse temporal mapping. Group aggregation checks preserve all source contexts, null evidence dates and original provenance without mutating rows. Browser checks exercise the vertical calendar rail with explicit folds, named lanes, dense-day sources/persons/parts, complete lists, nested evidence and Bestand navigation, original datings, editorial context provenance, invalid/undated access, stable selection geometry and responsive dialog/column placement. Network checks preserve plot size, visible node geometry and zoom during shared-panel selection. Smoke canaries cover calendar anchors, source/list controls and complete evidence paths. The execution checkpoint in plan.md records the checked snapshot and scope.
