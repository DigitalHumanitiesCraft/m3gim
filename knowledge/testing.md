...
A shared UI change must be verified in every current consumer. Exercise a real dense case before and after opening, replacing and closing selection, on narrow and wide hosts. Compare horizontal and vertical content bounds and the host's drawing dimensions; confirm that long lists scroll internally and that close/back controls remain reachable by keyboard. DOM attachment or a preserved scroll offset alone does not establish visibility. The existing Chronik and Netzwerk suites provide consumer-specific regression evidence. The Netzwerk suite does not yet automate replacing one open selection with another; record that check separately when verifying this behaviour. A future consumer needs equivalent evidence before a shared change is considered verified.
...
Chronik data regressions verify source-specific document/statement separation in `NIM_023_5`, the Zürich context of `NIM_007_11`, undated contractual places in `NIM_005_16`/`NIM_005_17`, identity preservation, typed parts and exact evidence. Pure calendar checks cover precision, leap years, ranges, invalid/reversed values, qualifiers, natural grouping, coarse-date preservation, compressed gaps and inverse temporal mapping. Group aggregation checks preserve all source contexts, null evidence dates and original provenance without mutating rows. Source-summary checks cover the real `NIM_005_25` values, precise calendar coverage, distinct-value counting, intervals, qualified boundaries, uncertainty, year-only dates and unchanged inputs.

Browser checks exercise the vertical calendar rail with explicit folds, named lanes, dense-day sources/persons/parts, complete lists, nested evidence and Bestand navigation, original datings, editorial context provenance, invalid/undated access, stable selection geometry and responsive dialog/column placement. The 26 July 1953 group is the dense-day case; the 1924/1925 source range checks that aggregation retains the original extent in the main lane. The real `NIM_005_25` year preview must show `25 Datierungen · Jänner–Dezember 1963`. The sticky current year must follow both year navigation and scrolling. Check horizontal and vertical bounds of every layouted group child in the shipped corpus, including decade/year scales at 390, 800 and 1366 pixels with details open and closed; one dense day does not represent every wrapping case. Compare all group positions, heights and scroll offset across selection. The largest historical gap must stay visibly marked and compact in its initial state. These examples protect source and interaction behaviour; exact pixel constants are implementation choices.

Network checks preserve plot size, visible node geometry and zoom during shared-panel selection. Smoke canaries cover calendar anchors, source/list controls and complete evidence paths. The execution checkpoint in plan.md records the checked snapshot and scope.
