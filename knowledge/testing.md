...
The progressive-detail regressions additionally check explicit text versus typed search actions and counts, unchanged Enter selection during pointer hover, self-contained chips and complete source access after opening disclosures. Place coverage includes overview → role/all sources → document evidence → overview, with unchanged global filters, role attribution on every evidence row and all 42 Zürich documents/66 statements retained. Dashboard, register and network disclosures keep full membership; opening a source list does not itself filter. Long opened lists must retain the complete sticky heading and close/back actions at both 390 and 1440 pixels. Derived and unresolved stage-part qualifications remain visible before expansion (E-299).
