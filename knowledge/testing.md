...
Chronik data regressions verify source-specific document/statement separation in `NIM_023_5`, the Zürich context of `NIM_007_11`, undated contractual places in `NIM_005_16`/`NIM_005_17`, identity preservation, typed parts and exact evidence. Pure calendar checks cover precision, leap years, ranges, invalid/reversed values, qualifiers, natural grouping, coarse-date preservation, compressed gaps and inverse temporal mapping. Group aggregation checks preserve all source contexts, null evidence dates and original provenance without mutating rows.

Browser checks exercise the vertical calendar rail with explicit folds, named lanes, dense-day sources/persons/parts, complete lists, nested evidence and Bestand navigation, original datings, editorial context provenance, invalid/undated access, stable selection geometry and responsive dialog/column placement. The 26 July 1953 group is the dense-content case; the 1924/1925 source range checks that aggregation retains the original extent in the main lane. Check both horizontal and vertical lane bounds with the detail open and closed. The largest historical gap must stay visibly marked and compact in its initial state. These examples protect source and interaction behaviour; exact pixel constants are implementation choices.
