...
1. **Tab-Durchlauf** über die in `smoke.py` fest gelistete Auswahl (`bestand`, `chronik`, `statistik`, `indizes`, `netzwerk`) — keine JS-Errors, DOM rendert nicht-leer. Diese Liste ist enger als der reale `VISIBLE_TABS`-Satz (`bestand`, `chronik`, `statistik`, `indizes`, `mobilitaet`, `netzwerk`, `korb`): der seit E-109/E-111 sichtbare Mobilitäts-Tab (D3-geo-Karte) und der Korb stehen nicht im Smoke-Loop. Der Mobilitäts-Tab ist stattdessen per Browser-Sichtprüfung belegt; ihn in den Smoke-Loop aufzunehmen ist ein offener Nachzug (siehe [plan.md](plan.md)). Versteckte Perspektiv-Tabs (Mobilitäts-Atlas, Repertoire, Biogramm) werden bewusst nicht angesteuert (E-81).
