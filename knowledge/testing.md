...
E-301 adds source-fidelity checks across original recording, JSON-LD, RDF expansion and frontend projection. `test_70_source_fidelity.py` compares resolvable raw source rows and property-level provenance with the actual index cells and cached Wikidata properties. It checks that source values remain literals after RDF expansion and local roles remain on local mention nodes. Transform unit cases cover missing values, conflicting identities, unapproved authority candidates and malformed composites. Frontend cases cover separate names, explicit work/part bindings and the distinction between document dates, statement dates, ranges and qualifiers. Actual browser downloads complement structural export tests. A passing check of transformation fidelity does not establish that the original cataloguing or authority statement is historically correct.
...
|---|---|
| Shared application | Smoke visits router views and checks drawing, data stamps, filters, URL roundtrips, source navigation and console errors. WARN, FAIL or unexpected browser errors fail the run. Focused cases exercise shared search, empty cuts, explicit filter replacement, narrow sidebar controls and accessible data-state information. A real HTTP-cache regression seeds an incompatible filter module, confirms cache reuse, then opens and reloads the current dashboard at both site-root and repository-prefix addresses. It checks versioned transitive requests, shared filter state and navigation to Orte. |
| HTML/CSS integration | Shared metadata, navigation, footer and asset order match the page catalogue; standalone synchronization is deterministic and its check mode writes nothing. Browser cases preserve the exact filtered URL and focus through the skip link, verify named panels and visible matrix intersections, and exercise bounded internal scrolling with open/closed sidebars at short desktop and narrow sizes. JavaScript-disabled and failed-import cases expose usable fallback links and recovery. A held module request verifies that tab navigation becomes operable only after startup and that the first register click works. |
...
| Bestand interaction | `NIM_023_5` supplies complete title selection/copying at two widths. `NIM_004_3` supplies the single München tooltip with admitted coordinates and a separate Wikidata link, keyboard focus, description and dismissal. Quality notes on grouped dates are keyboard-accessible; ordinary dates add no empty focus stop. Opening, switching and closing records preserve the activated row's position at 1366 and 800 pixels. |
...
| Shared query/search | Literal sources establish free-text Zürich 45, typed Zürich 42, bound performance-place zero and receiving 13. Legacy co-mention remains two/15 respectively. Tests check same-entry person/institution binding, missing roles, invalid predicates, complete candidate inventories, deterministic URL state and history. Browser checks cover draft versus commit, typed collisions, keyboard operation, removal, reload and source inspection outside the cut. |
| Dashboard | Treemap partition, matrix cell ID sets and binding status, document-date bins with separately preserved qualified and interval datings, source-conserving place-role Sankey, inclusive/exclusive UpSet and A/B universe/denominators use shipped and negative fixtures. Each aggregate must retain original dimension witnesses and complete source IDs. Real-browser tests operate two independent panels, same-chart pairs with different conditions, highlights retained across panel replacement, selection union/removal, explicit filtering, temporal focus, source detail, persistence and actual downloads. Full value lists and zero/missing cases remain reachable. |
| Dashboard geometry and access | Inspect real dense panels at 390, 800, 1440 and 2048 pixels before/after selection. Actual remaining container width governs stacking beside the 300-pixel detail. Check complete internal scrolling, local focus/zoom preservation, visible selected state, keyboard equivalents for drawing marks and Escape return to the correct originating panel. |
| Register navigation and shared chips | Nordwestdeutscher Rundfunk reaches its six literal source-backed documents through the institution facet with mouse or keyboard; a document query retains its three matching records. Wikidata keyboard activation leaves the filter unchanged. Register/co-mention jumps preserve the shared query and typed identity; the single shared search has a separate open action. At 390, 800 and 1440 pixels the complete Tristan document co-mention lists, internal scrolling, geometry and keyboard return remain available. The first Indizes click exposes all four registers (E-293). |
| Orte source projection and navigation | Source-key set equality against every loaded place row verifies lossless preservation and mirror deduplication. Warschau at `NIM_005_15`, source row 903, counts once. `NIM_023_5` keeps the undated Wuppertal place reference separate from 26 April 1953 (document) and 4 April 1953 (dated content statement). Boundary cases retain distinct addresses/rows, qualified dates and caveats. Browser checks cover complete source lists, distinct document counts, shared facets, local selection, explicit filtering, Chronik transfer, source navigation, missing coordinates, failed geometry, typed search focus and 390/800/1440-pixel detail geometry with scrolling and focus return. Selecting an already-visible Wien must centre it after the detail geometry settles; resizing preserves useful zoom. Zürich has no admitted coordinates and must retain all source evidence without a fabricated marker. Map keyboard navigation, escaping and undated context remain covered. |
...
The progressive-detail regressions additionally check explicit text versus typed search actions and counts, unchanged Enter selection during pointer hover, self-contained chips and complete source access after opening disclosures. Place coverage includes overview → role/all sources → document evidence → overview, with unchanged global filters, role attribution on every evidence row and all 42 Zürich documents/63 original statements retained. Dashboard, register and network disclosures keep full membership; opening a source list does not itself filter. Long opened lists must retain the complete sticky heading and close/back actions at both 390 and 1440 pixels. Explicit work/part bindings and unassigned parts remain distinct; E-301 removes the earlier single-work inference.
...
The E-300 interaction regressions additionally cover the single global clear action and undo/redo labels, regional map focus with stronger manual zoom retained, paired presets over an actually narrowed filter, restored panel addresses, keyboard menu dismissal and a real aggregate download. The Zürich text-search receipt `NIM_007_5_8` has no work information: selecting it must explain the empty companion matrix highlight while preserving its exact source and the shared filter. Synthetic coverage fixtures separate reference-only A records, missing work/composer data, absent binding and off-page cells. Browser checks retain matrix content after excessive saved page values and verify independent sticky header axes.
