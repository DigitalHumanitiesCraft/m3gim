...
Guided Bestand feedback adds browser regressions for selecting and copying the complete title of `NIM_023_5` at two viewport widths while preserving the open row, plus signature and keyboard toggling. The Potsdam tooltip at `NIM_003_1_1` is checked as one unfragmented overlay, including the nested provenance pill, keyboard focus, preserved accessible descriptions and dismissal. These checks cover the reported interaction defects; the project lead's interpretation and source inspection remain separate acceptance evidence in [plan.md](plan.md).
