...
The application includes the continuous scaled Chronik and a shared Chronik/Netzwerk selection detail component (M6h). The project lead approved implementation after reviewing the interactive wireframe. Technical checks and visual review against the actual corpus are complete; their scope is recorded below. Guided acceptance of the new presentation remains pending; version 0.9 has not been accepted.

**Next executable step:** inspect the implemented Chronik with the project lead: proportional intervals, crowded days, the distinction between document dates and dated statements, and the shared selection details. Register search unification, register details and the institution-chip fix remain separate open work.
...
| M6h Continuous time and shared selection | Scaled uninterrupted axis, proportional ranges, density groups and shared Chronik/Netzwerk detail component | Implemented and independently reviewed; targeted technical checks passed, guided acceptance pending |
...
Resume through the root action layer and this document's first section. The current implementation and verification checkpoint is M6h; Git identifies its local commit. Source tables, generated datasets and vocabulary remain unchanged by this UI refactoring. The existing pre-plan stash was left untouched; it is not a pending step and must not be applied automatically.
