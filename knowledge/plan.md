...
8. Karten-Ansicht entitätszentriert erweitern. Ein Institutionsfilter (etwa Bayreuther Festspiele) soll die zugehörigen Orte als Punkte zeigen, nicht als Reisepfeile zwischen Orten. Je Ort die daran hängenden Personen, Werke und Dokumente plus die zeitliche Dimension auflösen (Orts-, Werk- und Personen-Ebene in der Karte). Das ergänzt die heutige biografische Trajektorienkarte (E-111) um eine facettierte WO-und-wer-Sicht und braucht `institution` als zusätzliche Filter-Facette (heute nicht in `FILTER_FACETS`).
