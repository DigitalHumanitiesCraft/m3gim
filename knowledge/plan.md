...
| Smoke-Loop um Mobilitäts-Tab erweitern | offen | `tests/frontend/smoke.py` iteriert eine fest gelistete Tab-Auswahl (`bestand`, `chronik`, `statistik`, `indizes`, `netzwerk`), die seit E-109/E-111 hinter dem realen `VISIBLE_TABS` zurückhängt: der sichtbare Mobilitäts-Tab (D3-geo-Karte) und der Korb fehlen im Loop. Der Mobilitäts-Tab ist bisher nur per Browser-Sichtprüfung belegt. Ihn in den Smoke-Durchlauf aufnehmen, sobald ein Playwright-Lauf verfügbar ist (Voraussetzung d3-CDN plus die lokale `docs/data/geo`-Geometrie). |
