...
The application includes the Chronik lane revision and its right detail panel (M6f/M6g). The project lead authorised implementation in the existing UI with the complete linked corpus after reviewing a wireframe. Technical evidence is recorded below; guided acceptance of the new Chronik is pending. Version 0.9 has not been accepted. This plan owns current work and acceptance evidence, while [journal.md](journal.md) owns decision provenance.

**Next executable step:** discuss a continuous, scaled time axis with the project lead. The latest feedback asks for interval length to express duration and smaller entries to attach to that same axis. The existing chronology still orders content-sized date groups; its short interval marker does not fulfil that new geometric requirement. No further time-scale redesign is implemented. Register search unification, register details and the institution-chip fix remain open below.
...
| M6g Stable Chronik details | Right detail column, complete side lists, fixed upper-right legends and open Verknüpfung section | Implemented; targeted verification below, latest time-scale redesign remains a proposal |
...
| Chronik details, M6g | Ten targeted Chronik browser cases pass, including dense lists, source navigation, keyboard return and dialog/desktop resizing. Two map browser checks pass. Root inspected 1536-pixel sparse/dense Chronik and 390-pixel dialog screenshots; independent review checked detail context and selection. All 673 Node checks and seven knowledge-integrity checks passed; no skips. The ten Chronik browser checks passed again after integration. Guided acceptance and the newly requested duration geometry remain open. |
...
Resume through the root action layer and this document's first section. The last application commit is `9b2d1da`; this close changes knowledge and action-layer routing plus inclusion of AGENTS in the existing integrity check. Source tables, datasets, vocabulary and application files remain unchanged. The existing pre-plan stash was inspected by name and left untouched; it is not a pending step and must not be applied automatically.
