...
| Statistik-Dashboard (interaktiv) | erledigt (Runde 1) | E-122 (2026-06-22). Statistik vom Read-only-Showroom auf die geteilte Sidebar-Shell umgebaut (Panel-Grid + Ansichten-Toggles); ein record-basierter Zeitraum-Filter rechnet alle sechs Panels gleichzeitig neu — erster realer Baustein des Cross-View-Filters (E-117/Milestone 4). Form an die Datenlogik angeglichen (Dokumenttypen Donut→Bar, Personen „nach Rolle" datengetrieben statt namensbasierter Kategorie), Finanz-Beträge bewusst nicht summiert, Währungs-Artefakte als „unklar"-Gruppe (QF-14). smoke 25/25, Filter-Check 892→73, JS unberührt. Rein Frontend, Live-Deploy operator-gated. Offen (Stufe 2): Geografie→Karte, Histogramm unter den Filter, Long-Tail-Bars, Panel-Header/Farben, sechs Panels in ~vier Gruppen. |
