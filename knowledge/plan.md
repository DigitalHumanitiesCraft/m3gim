...
The subsequent user-reported startup failure was reproduced in the in-app browser despite current files being served correctly. JavaScript URLs were unversioned and allowed incompatible cached modules. The shared HTML generator now versions the complete module graph through an import map (E-298). Reloading the affected browser restores both dashboard panels. A regression with a real populated HTTP cache fails before the correction and passes afterwards at the site root and a repository prefix, retaining the Zürich cut through reload and navigation. The final focused run passes 145 HTML, browser and knowledge checks; standalone synchronization also passes without site-packages. The earlier full-suite result above describes `2f6eebd`.
