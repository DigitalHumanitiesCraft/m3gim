...
Sidebar. One column of fixed order in every view, composed in `docs/js/ui/sidebar.js` from the parts beside it (E-158, E-166, E-250). The order is search, Zeitraum, shared facets, view-owned sections, legend, with three rules, before facets, sections and legend. The search names its searchable fields in the placeholder and is absent where it cuts nothing (E-169). The result line is the root row of the Dokumenttyp tree, not a block of its own. Dokumenttyp comes first, then the entity facets, each with the dot of its family. Selected values stand in a strip over the work area rather than in the column, so a filter change never makes the column jump (E-182, E-192).
