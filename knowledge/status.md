...
### Session 47 — Netzwerk-Tab Hygiene-Runde: Tests, Tokens, Edge-Feedback, Modulsplit

Konstruktiv-kritische Nachbereitung der Session-46-Viz (E-94). Fünf Arbeitspakete, die das Netzwerk-Tab von „funktioniert für Demo" auf „saubere Codebase" hochziehen:

- **Unit-Tests für die Pure-Functions.** Neue Test-Datei [`tests/frontend/netzwerk-geometry.test.mjs`](../tests/frontend/netzwerk-geometry.test.mjs) nutzt Node 18+ Built-in `node:test` + `node:assert/strict`. 42 Tests decken `classifyRing`, `isMalaniuk`, `isPureComposer`, `derivePersonKategorie`, `nodeEvidence`, `nodeColor`, `computeLayout` (inkl. Determinismus + alphabetischer Winkel-Reihenfolge + Umlaut-Normalisierung + Radius-Cap), `computeCoOccurrence` (inkl. minShared-Threshold, maxEdges-Cap, Determinismus bei Ties, Malaniuk- und Komponisten-Filter) und `labelGeometry` ab. Lauf: `node --test tests/frontend/netzwerk-geometry.test.mjs`. Enabler: neue `docs/js/package.json` mit `{"type":"module"}` markiert den Baum als ES-Modul fürs Node-seitige Loading — Browser ignorieren die Datei.
- **Sophokles-Datenartefakt dokumentiert.** Live-Abfrage über Playwright hat gezeigt, dass in Kategorie „Andere" nur eine Person landet (Sophokles, Rollen `['erwähnt', 'aufführung']`, Wikidata Q7235). Grund: In `M3GIM-Verknüpfungen.xlsx` Zeile 1208 trägt Sophokles als Person die Rolle `Aufführung` — semantisch falsch (sein Werk wurde aufgeführt, er selbst nicht). Neuer Eintrag § 15 in [`knowledge/xlsx-fixes.md`](xlsx-fixes.md) mit Source-Fix-Vorschlag (Rolle auf `Vorlage` oder `Verfasser` ändern). Bewusst **kein** `aufführung → Bühne`-Sondermapping im Code — das würde für Werk-Autor:innen falsche Einordnungen produzieren.
- **Kategorie-Farben als Design-Tokens.** Sieben neue `--color-netzwerk-*`-Tokens in [`variables.css`](../docs/css/variables.css) (Produktion/Bühne/Vermittlung/Korrespondenz/Presse/Erwähnt/Andere). `NETZWERK_KATEGORIEN` in `_netzwerk-geometry.js` liefert jetzt `var(--color-netzwerk-X)`-Strings statt Hex-Literals. `drawCanvas` nutzt `.style('fill', …)` statt `.attr('fill', …)`, weil SVG-Attribute `var()` nicht auflösen. Verifiziert: aufgelöste Fills kommen als `rgb(…)` korrekt durch. E-77 (Cluster-Farben als CSS-Tokens) wird damit für den Netzwerk-Tab eingelöst.
- **Edge-Hover mit Endpunkt-Highlight.** Neue `applyEdgeHighlight(kind, data)`-Funktion im Canvas-Modul hebt bei Hover über eine Ko-Okkurrenz-Kante genau die beiden Endpunkt-Knoten hervor und dimmt alle anderen plus alle übrigen Kanten. Für AgRelOn-Radials dasselbe mit dem einen äußeren Endpunkt. Neuer `.netzwerk-link--active` CSS-Modifier. Im Gegensatz zum Knoten-Hover (der *alle Nachbarn* beleuchtet) wird hier nur die eine konkrete Verbindung sichtbar — „wer ist mit wem verknüpft, warum". Pin-Zustand wird respektiert (Hover wird ignoriert, wenn ein Knoten gepinnt ist).
- **`netzwerk.js`-Split in drei Module.** Von 1095 auf **484 Zeilen** (-56 %). Sidebar ausgelagert nach [`_netzwerk-sidebar.js`](../docs/js/views/_netzwerk-sidebar.js) (288 Zeilen, reine UI-Produktion mit `state`/`actions`-Vertrag), Canvas-Rendering inklusive Hover/Highlight/Zoom nach [`_netzwerk-canvas.js`](../docs/js/views/_netzwerk-canvas.js) (395 Zeilen). Orchestrator behält State, Filter-Anwendung, Detail-Panel, Telemetrie und Zeitfenster-Index. Bindeglied: `_zoomRefs = {behavior, svg}` (wird von `drawCanvas` mutiert, von den Zoom-Controls via Closure gelesen) und `_canvasActions = {getSelected, setSelected}` (Getter liest aktuellen Pin-Zustand, Setter kapselt `_selected`-Zuweisung plus `drawDetail()`-Aufruf). Bonus-Fix: Detail-Close-Button ruft jetzt `applyHighlight(null)` auf — das alte Panel ließ die Hover-Dim-Kette nach dem Schließen stehen.

Verifikation: 42/42 Node-Unit-Tests grün, 18/18 Playwright-Smoke grün, Interaktionstests bestätigen Ko-Okk-Hover (1 Kante aktiv + 2 Endpunkte fokussiert), AgRelOn-Hover (1 Link aktiv + Endpunkt fokussiert), Pin-State (Detail-Panel sichtbar), Zoom-Buttons (transform `scale(1.4)` nach Click).
