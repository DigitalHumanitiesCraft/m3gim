...
- **Abgrenzung von `sammlung` gegen `konvolut`.** Zu entscheiden ist, ob `sammlung` ein aggregierendes Erfassungsartefakt ist oder ein eigener semantischer Typ, also ob die beiden Begriffe einen physischen Umschlag gegen eine thematische Zusammenstellung unterscheiden oder dasselbe meinen. Im Entscheidungsblatt ausdrücklich vertagt; die Prüfung gegen [`../vocab/m3gim.ttl`](../vocab/m3gim.ttl) zeigt `m3gim-dft:sammlung` weiterhin ohne `skos:broader` auf `konvolut`, mit einer Editorial Note, die genau diese Offenheit festhält, und der Typ ist im ausgelieferten Datensatz belegt. Solange das nicht geklärt ist, bleibt unklar, ob beide Typen im Modell getrennt geführt werden müssen. Grundlage in [architecture-decisions.md](architecture-decisions.md) § Offene Modellentscheidungen.
- **Status der Personen-Rolle `protagonist`.** Vermutlich liegt ein Erfassungsfehler vor, weil `protagonist` eine Bühnenrolle bezeichnet und damit in den Typ `rolle` (`m3gim:StageRole`) gehört, während der Wert heute als Rolle einer Person geführt wird. Zu entscheiden ist, ob der Eintrag umzuklassifizieren ist und ob ein Pipeline- oder ein Quellen-Fix im XLSX vorzuziehen ist. Im Entscheidungsblatt vertagt, im ausgelieferten Datensatz unverändert vorhanden. Solange der Wert dort steht, führt der Rollenbestand eine Partie unter den Akteursrollen. Grundlage in [architecture-decisions.md](architecture-decisions.md) § Offene Modellentscheidungen.
- **Doppelrepräsentation der Partie.** Die Property `m3gim:partie` führt dieselbe Partie noch einmal als Literal am Werk, ohne Verbindung zur `StageRole`-Entität. Im Entscheidungsblatt vertagt; im ausgelieferten Datensatz stehen beide Formen nebeneinander. Zwei Repräsentationen derselben Sache nebeneinander löst auch ein besserer Name nicht auf, der Umbau der Kennung räumt die Verwechslung allein auf der Ebene der Adresse ab. Grundlage in [rename-map-paket-2.md](../data/reports/rename-map-paket-2.md) § 3.4.
- **Umsetzung des Occurrence-Modells.** Das Zielmodell ist mit E-128 entschieden, seine Umsetzung im Entscheidungsblatt vertagt. Offen sind die Pipeline-Gruppierung nach `(archivsignatur, folio, datenpunkt_id)` zu `m3gim:Occurrence` und der Erfassungs-Rollout im Team. Die Prüfung des ausgelieferten Datensatzes zeigt `m3gim:datenpunktId` nur vereinzelt befüllt und keinen Occurrence-Knoten im Graphen. Solange das so bleibt, zeigen der Verknüpfungen-Tab und der geteilte Filter weiterhin den weiten Schärfegrad „im selben Dokument genannt", der enge Schärfegrad „im selben Auftritt" bleibt unerreichbar, und die Bindung von Person, Partie und Aufführung fehlt im Datensatz. Grundlage in E-125, E-127 und E-128 sowie unten in § Auftritts-Occurrence und Forschungsdatenstufe.
- **Vertragsstatus.** Der Rollenwert `nicht eingehalten` belegt heute Stellen in mehreren Properties, während `m3gim:contractStatus` und `m3gim:realized` seit E-99 deklariert und unbefüllt sind. Die Umstellung setzt die offene Klärung mit dem Erfassungsteam voraus und ist deshalb vertagt; die Prüfung des ausgelieferten Datensatzes bestätigt beide Properties als leer. Die Anforderung A1 des Frontend-Vertrags verlangt für jeden Rollenwert eine Anzeigeform aus den Daten, und `nicht eingehalten` ist die dokumentierte Ausnahme ohne eigenes Concept. Grundlage in [date-role-model-befund.md](../data/reports/date-role-model-befund.md) § 6.
- **Rollenproperties der AgRelOn-Beziehungen.** `_maybe_add_agrelon` in `scripts/transform.py` setzt an jeder erzeugten Beziehung `agrelon:hasSubject` und `agrelon:hasObject`, auch an der als `owl:SymmetricProperty` deklarierten `agrelon:HasCorrespondent`, für die AgRelOn `agrelon:hasSubjectObject` vorsieht. Der zweite Punkt betrifft die Orientierung von `agrelon:IsHasPatron`, wo nach der Lesart des AgRelOn-Kommentars die `isPatronOf`-Seite als `hasSubject` stünde, während die Pipeline dort die Nachlassbildnerin einsetzt. Am 2026-08-22 gegen `scripts/transform.py` geprüft, der Zustand ist unverändert. Solange offen, tragen die erzeugten Beziehungen eine Rollenzuweisung, die die Ontologie anders vorsieht. Grundlage in [architecture-decisions.md](architecture-decisions.md) § Offene Modellentscheidungen.
- **Granularität der Korrespondenz-Beziehung.** Bei Korrespondenz wird pauschal `agrelon:HasCorrespondent` erzeugt. Optionen sind (a) beide Richtungen erzeugen (symmetrisch), (b) feinere Prädikate `HasAddressee` und `HasSender` nutzen, (c) den Status quo belassen und im Frontend beide Seiten zusammenführen. Am 2026-08-22 gegen `scripts/transform.py` und den Datensatz geprüft; `AGRELON_MAPPING` führt `absender`, `empfänger` und `adressat` auf `HasCorrespondent`, während die Rolle `verfasser`, die im Bestand die Absenderseite trägt, ohne Zuordnung bleibt und `absender` im Datensatz nicht vorkommt. Folge ist, dass die UI-Sektion „KORRESPONDENZ" nur einen Partner zeigt und der zweite unter „PRODUKTION VERFASSER" steht. Grundlage in [architecture-decisions.md](architecture-decisions.md) § Offene Entscheidungen.
...
- **Eigener Rollenindex für `m3gim:StageRole`.** Ein Index analog zu den bestehenden für Werke, Orte, Personen und Organisationen trägt mindestens die Spalten `m3gim_id`, `name`, `belongsToWork`, `voiceType` und `wikidata_id`. Voraussetzung ist ein neues Rollenindex-XLSX vom Erschließungsteam. Am 2026-08-22 gegen `data/google-spreadsheet/` geprüft, dort liegen die Arbeitsmappen für Objekte, Personen, Organisationen, Orte, Werke und Verknüpfungen ohne einen Rollenindex. Solange er fehlt, tragen die `m3gim:StageRole`-Knoten im Datensatz allein ihren Namen, ohne Werkbindung und ohne Stimmfach. Grundlage in [architecture-decisions.md](architecture-decisions.md) § Offene Modellentscheidungen.
...
- **Reihenfolge mehrerer Datierungen (A2).** Zwei Ordnungen sind zu ersetzen. Die Ordnung zwischen Rollen ersetzt ein Rang am Rollenbegriff, etwa ein `skos:notation` oder eine eigene Property, die die heutige Listenreihenfolge von `TYPED_DATE_PROPS` abbildet. Die Ordnung innerhalb eines Records ersetzt entweder eine zugesicherte Array-Reihenfolge, die der Loader nicht umsortiert, oder eine explizite Ordnungszahl je Datierung. Ohne den Rang fällt die Auswahl der Sekundärdatierung auf den frühesten Wert zurück, was für einen Record mit Generalprobe und Aufnahme das Generalprobendatum liefert. Grundlage in [frontend-date-contract.md](../data/reports/frontend-date-contract.md) § 3.
- **Bezugsebene je Datierung (A3).** Der Vertrag braucht einen eigenen kontrollierten Term je Datierung mit mindestens drei Werten, der Datierung des Objekts, der Datierung eines bezeugten Ereignisses und der Datierung einer Erwähnung. Offen sind zwei Punkte, ob `nicht eingehalten` und `rahmenveranstaltung` eigene Werte bekommen oder als Erwähnung geführt werden, und ob der Term an der Datierung selbst sitzt oder an der Bezeugungslogik aus `m3gim:attests` und `m3gim:Occurrence`, was seine Verfügbarkeit an die Occurrence-Ebene bindet. Ohne die Trennung wandern erwähnte Daten in den Zeitstrahl, der heute allein die bezeugten Ereignisse zeigt. Grundlage in [frontend-date-contract.md](../data/reports/frontend-date-contract.md) § 3.
- **Zeitanker je Record (A4).** Der Vertrag muss genau eines von zwei Dingen zusichern. Entweder bleibt `rico:date` als einwertiger Zeitanker am Record bestehen und wird aus der zusammengeführten Menge deterministisch abgeleitet, oder die zusammengeführte Menge trägt eine ausgezeichnete Datierung und das Frontend bekommt eine einzige Funktion, die diesen Anker liefert. Die geteilte Facette `zeitfenster` wirkt in fünf Ansichten, die das Jahr heute auf vier verschiedene Arten auflösen; eine Zusammenführung ohne diese Zusicherung macht daraus vier verschiedene Filterergebnisse. Grundlage in [frontend-date-contract.md](../data/reports/frontend-date-contract.md) § 3.
