...
The Indizes orchestrator (`views/indizes.js`) owns the active register, local name lookup, sorting, shared-cut entries and selection. `indizes-data.js` computes entries, roles and co-mentions without DOM. `indizes-detail.js` builds complete expandable lists and source-specific relationship links inside the shared selection shell. Properties, the visibly labelled single-work role inference and document co-mentions have separate sections. An entry jump clears only the local lookup. The record count links to the corresponding shared facet; network and map actions carry view-specific context (E-293).

The Orte view retains the `karte` route and the modules `views/karte.js`, `karte-data.js` and `karte-map.js`; `karte-detail.js` supplies its source content to the shared selection shell. `buildOccurrences` preserves each source row, address and original annotation date. It merges the direct-place and annotation representations only when record, source sheet/row, original place and role match. `date` belongs exclusively to a place annotation; `documentDate`, `recordDate` and `recordYear` retain document context separately. This is a frontend projection with unchanged JSON-LD and no occurrence grouping. `groupPlaces` collects evidence for navigation and counts distinct documents per place and role. `occurrencesInCut` resolves the full `recordsFor` cut and any explicit place-role restriction. The optional sidebar lookup narrows list and map together. Register navigation resolves an entity to its shared facet; the former local entity picker is removed. Native list buttons and keyboard map selection open the same complete place detail without modifying filters. Explicit detail actions filter the place or transfer that cut into Chronik. The map uses local country geometry and D3; equal-size markers remain constant under zoom, labels avoid collisions, viewport coverage stays visible and geometry failure does not prevent evidence access.
