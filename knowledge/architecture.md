...
The Chronik resolves the common document set through `recordsFor` and projects exactly those records through `chronik-timeline-data.js`. `buildChronikTimeline(store, records)` retains source identities, dated statements, document context, typed parts, notes and original provenance. `chronik-time-layout.js` independently parses exclusive UTC calendar extents, groups labels into fixed pixel cells without merging rows, and derives adaptive calendar ticks. `chronik-axis.js` draws one linear scroll surface, keeping all source marks, interval bands and compact groups in the DOM while refreshing only visible ticks. Label height never determines temporal distance. Scale and navigation affect only the display; the date projection and shared document cut stay unchanged. Malformed values and undated context remain reachable outside the scaled surface. `chronik.js` supplies compact source/entity summaries and complete evidence content.
