...
| `views/indizes.js` | Die vier Register als Grid-Explorer, mit eigener Sortierung, aufklappbarem Detail und Cross-Grid-Schnitt |
...
Unter 900 Pixel Containerbreite faltet sich die Filterspalte hinter einen Schalter, den `viewShell` als erstes Kind der Shell baut; unter 700 Pixel verliert die Bestandstabelle die Typspalte und die Zahlen der Entitätenspalte, die Chronik verengt ihre Jahresspalte. Der Druck zeigt jeden Konvolut offen, weil `bestand.js` auf `beforeprint` alle Köpfe öffnet und auf `afterprint` den Zustand zurückstellt; das Druck-Stylesheet nimmt Seitenleiste, Chip-Zeile, Sprungliste und Korb-Spalte heraus.
...
- `navigateToIndex(gridType, entityName)` springt in die Indizes, `navigateToView(tab, {recordId})` in eine Ansicht mit Kontext und schreibt dabei die Record-ID samt Filterteil in die Adresse, und `applyArchivFilter(facet, value)` schreibt einen Facettenwert in den geteilten Filter und wechselt in die Ansicht, die ihn zeigt. Jeder Record-Sprung aus einer Ansicht nimmt diesen Weg, kein Modul setzt `location.hash` selbst (E-208).
