...
| `views/_facet-sidebar.js` | Die geteilte linke Filterspalte: Status- und Legendenblock, optionale Freitextsuche, optionaler Scope-Umschalter mit Zaehlwerten, Facetten mit Mehrfachauswahl (Dokumenttyp als DFT-Baumgruppen), Zeitfenster, Schaerfegrad, Zuruecksetzen |
...
| `views/archive-holdings.js` | Bestand-Tab: Konvolute als dauerhafte Gruppenköpfe mit Meta-Chips (häufigste Dokumenttypen + Status-Mix) und Objektzeilen darunter, einzige Klapp-Interaktion ist das Record-Detail (E-158). Trägt die Scope-Regel `isOutOfScope` (E-157), die typisierte Erschließungsanzeige über `familiesForRecord` und die Zählwerte des Scope-Umschalters. Hierarchische Sortierung: Konvolute Signatur-stabil, Kinder innerhalb sortierbar (E-83). Die Filter liegen vollständig in `_facet-sidebar.js`. |
...
| `views/archive-inline-detail.js` | Vollbreites Record-Detail: Aktionsleiste (Korb, Schließen), Metadatenzeile Typ · Datum · Sprache · Umfang · Status, `rico:scopeAndContent` als Beschreibung, das Blockraster Produktion · Mitwirkende · Werk & Repertoire · Aufführungen · Ort & Ereignis · Im Dokument genannte Daten · Erwähnt · Weitere · Beziehungen · Finanzen mit Familien-Farbpunkt am Blocktitel und ein Fuß aus gebündelter Quellenangabe (`sourceSummary`) und zuklappbarer Verwaltung. AgRelOn-Dedup liest `rel.objectName`/`rel.objectWikidata`, nicht das rohe JSON-LD. `partitionRecord()`, `sourceSummary()` und `buildRoleChip()` sind die geteilten DOM-freien beziehungsweise geteilten Helfer, `buildRecordBlocks()` speist auch den Wissenskorb. |
...
- Bestand und Chronik sind eigene Top-Level-Tabs (früher Archiv-Sub-Toggle). Der Bestand trägt seine Bedienung seit E-158 vollständig in der geteilten linken Filterspalte (`_facet-sidebar.js`) mit Suche, Scope-Umschalter, den Facetten Dokumenttyp, Person, Ort, Werk und Rolle, Zeitraum, Schärfegrad und Zurücksetzen; die frühere Toolbar über der Tabelle ist entfallen. Die Chronik führt ihre Regler noch an ihrer bisherigen Stelle.
- **Leitprinzip „nur bearbeitet" als Default, umschaltbar über den Scope (E-116, präzisiert durch E-157):** im Scope `fein` rendern Konvolute ohne erschlossene Folios, Records ohne Verknüpfungen und Folios ohne Links nicht, und `EXCLUDED_DFT` hält Plakate und Tonträger draußen. Der Scope `gesamt` blendet nichts aus: die nicht erschlossenen Records und Konvolute erscheinen, in `getOrderedItems(showAll)` durchgereicht und in `renderRows` über `.archiv-row--unerschlossen` ausgegraut plus Badge „nicht erschlossen" markiert, und `isOutOfScope` lässt dort auch Plakate und Tonträger zu. So ist alles erreichbar, ohne den Erschließungsstand zu kaschieren; das Form-ist-Signal-Prinzip wird nicht aufgegeben, sondern als sichtbare Markierung statt als Ausblendung umgesetzt. Folios (reine Metadaten-Records) bleiben in beiden Scopes raus. Die Chronik filtert weiterhin ausschließlich über `unprocessedIds` ohne Umschalter, weil der lokale DFT-Ausschluss dort keine Treffer hat. Log-Stempel-Keys ergänzt um `erschliessung` (erschlossen|alle) und `nicht-erschlossen`.
- **Zählwerte im Bedienelement statt in einer Caption (E-156):** der Scope-Umschalter beziffert beide Scopes über `scopeCounts()`, der Statusblock der Sidebar trägt Zählstand, Deckung, Schärfegrad und belegte Teilmenge. Über der Tabelle steht weder eine Count-Caption noch ein Schärfe-Banner.
...
- **Konvolute als dauerhafte Gruppenköpfe** (E-82, E-158): Signatur, Titel, Zeitspanne, die häufigsten Dokumenttyp-Chips mit „+N weitere"-Rest und der Status-Mix (abgeschlossen/begonnen/zurückgestellt) stehen unter dem Konvolut-Titel, die Objektzeilen dauerhaft darunter. Der Kopf ist nicht klappbar und trägt kein Inline-Detail; die Chips folgen dem Scope, damit sie nicht gegen die gerenderten Zeilen driften.
...
- **Objektzeile und Spalten** (E-158): Signatur in Folio-Form, Titel, Typ, Datum, Erschließungsanzeige und der Korb in einer eigenen schmalen Spalte. Die Erschließungsanzeige zeigt je Inhaltsfamilie aus `CONTENT_FAMILIES` einen Punkt in der Familienfarbe, gefüllt bei Belegen und sonst offen, die Aufschlüsselung liegt im Tooltip; `familiesForRecord` leitet sie aus derselben `partitionRecord`-Zerlegung ab, die das Detail rendert. Die Spaltenheader sind klickbar, die Erschließungsspalte sortiert nach der Zahl der Verknüpfungen. Die Freitextsuche über Signatur, Titel, Dokumenttyp und Datum liegt in der Sidebar.
- **Inline-Detail für Records:** über die volle Breite, mit Aktionsleiste, Metadatenzeile, `rico:scopeAndContent` als Beschreibung, dem Blockraster Produktion · Mitwirkende · Werk & Repertoire · Aufführungen · Ort & Ereignis · Im Dokument genannte Daten · Erwähnt · Weitere · Beziehungen · Finanzen und einem Fuß aus gebündelter Quellenangabe und zuklappbarer Verwaltung (`processingNote`, `accessStatus`, `digitizationStatus`). Jeder Blocktitel trägt den Farbpunkt seiner Inhaltsfamilie, sodass die Legende aus der Nähe zur Erschließungsanzeige entsteht. Agents mit AgRelOn-Äquivalent werden aus „Mitwirkende" unterdrückt (Dedup-Filter liest das *flache* Loader-Format). Sprach-Kürzel (`en, fr`) werden über `formatLanguage()` zu lesbaren Labels aufgelöst. Alle Chips via `buildRoleChip()` mit Cluster-Farbe + Provenance-Pille.
- Bookmark in der Korb-Spalte jeder Record-Zeile und als eine der beiden gleich großen Aktionsschaltflächen des Details (toggeln in `store.korb`, Klick auf Tab öffnet den Wissenskorb).
- Der Reset-Knopf am Fuß der Sidebar setzt alle Filter gleichzeitig zurück.
...
- **Wirkung.** Die Mobilitäts-Strukturen tragen den sichtbaren Mobilitäts-Tab (E-111, D3-geo-Karte); die zurückgestellten Tabs Mobilitäts-Atlas, Repertoire und Biogramm bleiben damit ebenfalls tragfähig. Der Bühnenrollen-Block steht in [design.md](design.md). `dataQualityFlag` erscheint am Chip als Info-Symbol mit dem Wert im Tooltip; an den Record-Ebenen ohne Chip und für einen etwaigen Vertragsstatus fehlt die Anzeige noch.
...
`rolle` ist die Akteursrolle, nicht die `eventRole`; die `eventRole` speist ausschließlich die `sicht`-Facette. Die Erschließungs-Umschaltung (E-116) liegt seit dem Umbau als `scope` im geteilten Zustand, wirkt aber nur im Bestand; sie ist eine Darstellungsfrage dieser Ansicht, kein view-übergreifender Datenschnitt.
...
Gebaut sind `filter-state.js` mit dem `m3gim:filter`-Kanal, `buildToolbar` rückverdrahtet und Bestand und Chronik umgestellt, Netzwerk-Zeitfenster und Karten-Filter am selben State, der view-lokale `ROLE_TO_TYPE` durch `mobilityClusterFor` ersetzt, und der Schärfegrad als geteiltes Control mit Differenznennung. Die Chronik führt die Differenz über ein eigenes Banner, der Verknüpfungen-Tab über seine Caption, der Bestand seit E-156 über den Statusblock der Sidebar. Abgesichert ist die Kopplung durch den Smoke-Canary `m4:cross-view-filter` (Ort im Graph gesetzt, Bestand synchron gefiltert), durch `tests/frontend/filter-sync.test.mjs` für die Projektionen und den Loop-Guard und durch den logStamp-Vertrag des Verknüpfungen-Tabs.
...
Umgerüstet sind der Verknüpfungen-Tab und seit E-158 der Bestand, der die Spalte um die Freitextsuche, den Scope-Umschalter Feinerschlossen/Gesamt mit Zählwerten und die Dokumenttyp-Facette mit den DFT-Baumgruppen erweitert. Chronik, Karte, Netzwerk und Statistik lesen die Listenform korrekt und hängen unverändert am geteilten Filter, tragen ihre Regler aber weiterhin an ihrer bisherigen Stelle.
