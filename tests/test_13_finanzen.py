"""Finanzschicht-Invarianten (data-model.md § Financial layer).
...
# Im aktuellen Datenbestand belegt (data-model.md § Financial layer).
...
# (keine spekulative ISO-4217-Normalisierung, data-model.md § Financial layer).
ALLOWED_CURRENCIES = {
...
def test_finance_currency_in_known_set(records):
    ...
    assert not unknown, (
        f"Unbekannte Waehrungscodes (data-model.md § Financial layer erweitern oder Eintrag korrigieren): "
        f"{unknown[:5]}"
    )
