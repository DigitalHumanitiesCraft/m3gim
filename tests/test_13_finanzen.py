"""Finanzschicht-Invarianten (data-model.md § Finanzschicht).
...
# Im aktuellen Datenbestand belegt (data-model.md § Finanzschicht).
...
# (keine spekulative ISO-4217-Normalisierung, data-model.md § Finanzschicht).
ALLOWED_CURRENCIES = {
...
def test_finance_currency_in_known_set(records):
    ...
    assert not unknown, (
        f"Unbekannte Waehrungscodes (data-model.md § Finanzschicht erweitern oder Eintrag korrigieren): "
        f"{unknown[:5]}"
    )
