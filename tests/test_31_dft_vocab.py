...
Spec: data-model.md § Controlled vocabularies, journal.md E-101. The Verknuepfungen type `dokument` as
...
def test_sammlung_is_own_concept_without_broader(records, graph):
    ...
    (data-model.md § Controlled vocabularies)."""
    ...
    assert "skos:broader" not in concepts["m3gim-vocab:collection"], (
        "sammlung darf kein skos:broader tragen (data-model.md § Controlled vocabularies)."
    )
...
def test_new_concepts_scaffolded():
    """The new concepts are scaffolded in the model (data-model.md § Controlled vocabularies): placed
