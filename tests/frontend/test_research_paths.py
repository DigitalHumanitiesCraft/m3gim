...
def test_task4_canary_preserves_literal_basis_and_source_evidence(frontend_server):
    with sync_playwright() as pw:
        ...
        assert all(value in filter_text for value in ("Bayreuth", "Tristan und Isolde", "1954"))
...
@pytest.mark.frontend
def test_narrow_network_draws_nodes_inside_visible_plot(frontend_server):
    with sync_playwright() as pw:
        browser = pw.chromium.launch()
...
def test_network_download_matches_drawn_projection_and_document_cut(frontend_server, mode):
    with sync_playwright() as pw:
        browser = pw.chromium.launch()
        ...
        if mode == "projection":
            ...
            if sidebar_toggle.is_visible() and sidebar_toggle.get_attribute("aria-expanded") == "false":
