...
def test_bestand_tooltip_is_one_unfragmented_overlay(frontend_server, browser_context):
    ...
    record_id = quote("m3gim-data:NIM_004_3", safe="")
    page.goto(frontend_server + f"#bestand/{record_id}", wait_until="networkidle")

    munich = page.locator(".chip-action", has_text="München").first
    munich.evaluate("element => element.setAttribute('aria-describedby', 'record-context')")
    munich.hover()
    tooltip = page.locator(".tooltip-portal")
    expect(tooltip).to_be_visible()
    assert tooltip.count() == 1
    assert "Als Filter setzen" in tooltip.inner_text()
    assert "Koordinaten: 48.1375, 11.575" in tooltip.inner_text()
    assert tooltip.evaluate("element => element.parentElement === document.body")
    assert munich.get_attribute("aria-describedby") == "record-context app-tooltip"
    assert munich.evaluate(
    ...
    wikidata = munich.locator("xpath=..").locator(".badge--wikidata")
    wikidata.hover()
    expect(tooltip).to_contain_text("Bei Wikidata ansehen")
    assert wikidata.get_attribute("aria-describedby") == "app-tooltip"
    assert munich.get_attribute("aria-describedby") == "record-context"
