"""AgRelOn-Spec: Phase 4.8 — Agent-Agent-Relationen aus data-model.md § AgRelOn.
...
Mapping aus data-model.md § AgRelOn, Mapping M³GIM-Rolle → AgRelOn:
...
def test_has_employer_relations_from_arbeitgeber(records, xlsx_verknuepfungen):
    ...
    assert "typ" in df.columns and "rolle" in df.columns, (
        ...
        "(siehe knowledge/data.md § Compensations in the pipeline)."
    )
    ...
    if matchable == 0:
        pytest.skip(
            ...
            f"Record zugeordnet werden kann (verwaist, data.md § Compensations in the pipeline). Der "
